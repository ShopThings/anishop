...
class ReturnOrderStatusNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        return [
            'type' => UserNotificationTypesEnum::RETURN_ORDER_STATUS->value,
            'type_value' => UserNotificationTypesEnum::getTranslations(UserNotificationTypesEnum::RETURN_ORDER_STATUS->value, 'نامشخص'),
            'priority' => UserNotificationPrioritiesEnum::HIGH->value,
            'message' => 'مرجوع سفارش به کد' .
                ' <span>' . $this->orderCode . '</span> ' .
                'با کد مرجوع' .
                ' <span>' . $this->returnCode . '</span> ' .
                'به وضعیت' .
                ' <strong>' . $this->returnStatus . '</strong> ' .
                'تغییر یافت.',
        ];
    }
}
