...
const {canSubmit, errors, onSubmit} = useFormSubmit({
  ...
  SliderAPI.updateById(idParam.value, {
  ...
  SliderAPI.fetchById(idParam.value, {
    success: (response) => {
      ...
      if (response.data.place_in.value === SLIDER_PLACES.MAIN_SLIDERS.value) {
        if (response.data.options?.brand_id) {
          BrandAPI.fetchById(response.data.options.brand_id, {
            success: (response) => {
        ...
        if (response.data.options?.category_id) {
          CategoryAPI.fetchById(response.data.options.category_id, {
            success: (response) => {
        ...
        const orderByIdx = orderBy.findIndex((item) => item.value === response.data.options?.order_by)
        if (orderByIdx !== -1) {
