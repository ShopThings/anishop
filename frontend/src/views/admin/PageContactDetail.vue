  ...
  <partial-card>
    ...
    <template #body>
      <div class="p-3">
        ...
        >
          <template #content>
            <ul class="flex flex-col sm:flex-row flex-wrap gap-3">
              <li class="sm:flex sm:items-center">
                <span class="ml-2 text-gray-400 text-xs whitespace-nowrap">ارسال شده توسط:</span>
                <div class="text-sm mt-1 sm:mt-0 sm:inline-block">
              ...
              <li class="sm:flex sm:items-center">
                <span class="ml-2 text-gray-400 text-xs whitespace-nowrap">ارسال شده در تاریخ:</span>
                <div class="text-sm mt-1 sm:mt-0 sm:inline-block">
                  {{ contact?.created_at ?? '-' }}
                </div>
              </li>
              <li class="sm:flex sm:items-center">
                <span class="ml-2 text-gray-400 text-xs whitespace-nowrap">موبایل:</span>
                <div class="text-sm mt-1 sm:mt-0 sm:inline-block">
                  {{ contact?.mobile ?? '-' }}
                </div>
              </li>
...
onMounted(() => {
  ContactAPI.fetchById(idParam.value, {
    success(response) {
      ...
      // make contact as seen if not set by now
      if (contact.value && !contact.value?.is_seen) {
        ContactAPI.updateById(idParam.value, {
          is_seen: true,
        }, {
          success(response2) {
