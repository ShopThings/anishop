<template>
    <div role="status"
         class="p-4 space-y-4 border border-slate-200 divide-y divide-slate-200 rounded-lg shadow animate-pulse dark:divide-slate-700 md:p-6 dark:border-slate-700">
        <div class="flex items-center justify-between">
            <div>
                <div class="h-2.5 bg-slate-400 rounded-full dark:bg-slate-600 w-24 mb-2.5"></div>
                <div class="w-32 h-2 bg-slate-300 rounded-full dark:bg-slate-700"></div>
            </div>
            <div class="h-2.5 bg-slate-400 rounded-full dark:bg-slate-700 w-12"></div>
        </div>
        <span class="sr-only">در حال بارگذاری...</span>
    </div>
</template>
<script setup>
</script>
